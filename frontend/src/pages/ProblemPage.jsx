...
function ProblemPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [problemsResponse, companiesResponse, topicsResponse] =
          await Promise.all([
  ...
  const topicsWithCounts = topics.map((topic) => {
    const count = problems.filter(
      (problem) =>
        problem.tags &&
        problem.tags.some((tag) => tag.toLowerCase() === topic.toLowerCase())
    ).length;
              ...
              {difficulties.map((level) => (
                <div key={level} className="flex items-center">
                  <label className="flex items-center gap-2 cursor-pointer w-full">
                    ...
                    <span
                      className={`inline-block w-3 h-3 rounded-full ${getBadgeColor(
                        level
                      )}`}
                    ></span>
                  </label>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer w-full">
                    ...
                    <span
                      className={`inline-block w-3 h-3 rounded-full ${
                        s === "solved" ? "bg-green-500" : "bg-gray-500"
                      }`}
                    ></span>
                  </label>
                </div>
          ...
          {loading ? (
            ...
            <div className="alert bg-red-900/50 border border-red-700 shadow-lg">
              <div className="flex items-center">
                <svg
                  ...
                  className="stroke-current flex-shrink-0 h-6 w-6 text-red-400"
                  fill="none"
                  viewBox="0 0 24 24"
                >
                  <path
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 mt-2">
                There are no problems in the database yet
              </p>
            </div>
              ...
              <div className="grid grid-cols-1 gap-4 mb-6">
                {currentProblems.map((problem) => (
                  <div
                    ...
                    style={{
                      background:
                        "linear-gradient(135deg, rgba(30, 30, 30, 0.7), rgba(60, 60, 60, 0.3))",
              ...
              {totalPages > 1 && (
                <div className="flex flex-col items-center justify-center mt-4">
                  ...
                  <div className="text-sm text-gray-400">
                    ...
                    {Math.min(indexOfLastProblem, sortedProblems.length)} of{" "}
                  ...
                  {filteredTopics.map((topic) => (
                    <div key={topic.name} className="mb-2">
                      <label className="flex items-center gap-3 cursor-pointer text-white hover:bg-gray-700  rounded">
                        ...
                        />
                        <span className="text-sm flex-1">
                          {capitalize(topic.name)} ({topic.count})
                        </span>
                      </label>
                    </div>
                  ))}
